...
#print(DictOfCara)
tt = 0
file = open("CompleteSimlarityResults.csv", 'r')
reader = csv.reader(file)
counter = 0
...
while i < len(DictOfResults):
    category = " "
    if (float(DictOfResults[i].get('preoneAndCitation')) >= (0.25*stdAndmeanOfCitationScore)):
            citationCategoryPredicted.insert(i, "Pre Contiguous")
    elif (float(DictOfResults[i].get('citationAndPostOne')) >= (0.25*stdAndmeanOfCitationScore)):
